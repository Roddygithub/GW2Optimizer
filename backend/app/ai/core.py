...
class GW2AICore:
    ...
    async def compose_team(
        ...
        if not settings.AI_CORE_ENABLED:
            logger.warning("⚠️  AI Core disabled, using fallback", extra={"request_id": request_id})
            return await self._generate_fallback_composition(game_mode, team_size, preferences, request_id)

        # Try Mistral AI generation
        if self.mistral_api_key:
            try:
                composition = await self._generate_with_mistral(game_mode, team_size, preferences, request_id)
                ...
                logger.error(
                    ...
                    extra={"request_id": request_id, "error": str(e), "fallback": self.fallback_enabled},
                )
        ...
        logger.info("📋 Using rule-based fallback", extra={"request_id": request_id})

        return await self._generate_fallback_composition(game_mode, team_size, preferences, request_id)

    async def _generate_with_mistral(
        self, game_mode: GameMode, team_size: int, preferences: Optional[Dict[str, Any]], request_id: str
    ) -> TeamComposition:
    ...
    def _create_mistral_prompt(self, game_mode: GameMode, team_size: int, preferences: Optional[Dict[str, Any]]) -> str:
...
"""
    ...
    async def _generate_fallback_composition(
        ...
        composition = TeamComposition(
            ...
            weaknesses=["May lack flexibility for specific encounters", "Requires coordination", "Generic approach"],
            synergy_score=7.5,
            metadata={"source": "rule_based_fallback", "request_id": request_id, "preferences": preferences},
        )
    ...
    async def _enhance_with_ml(self, composition: TeamComposition, request_id: str) -> TeamComposition:
        """
        ...
        try:
            ...
            logger.warning(f"ML enhancement failed: {str(e)}", extra={"request_id": request_id})
