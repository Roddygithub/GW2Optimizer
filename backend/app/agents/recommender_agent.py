...
class RecommenderAgent(BaseAgent):
    ...
    async def validate_inputs(self, inputs: Dict[str, Any]) -> None:
        ...
        if inputs["profession"] not in valid_professions:
            raise ValueError(
                f"Invalid profession. Must be one of: {', '.join(valid_professions)}"
            )
        ...
        if inputs["game_mode"] not in valid_game_modes:
            raise ValueError(
                f"Invalid game_mode. Must be one of: {', '.join(valid_game_modes)}"
            )
    ...
    async def get_build_variants(
        ...
        for i in range(variant_count):
            ...
            variant_inputs["context"] = (
                f"{base_inputs.get('context', '')} "
                f"Provide variant #{i+1} with different trait/equipment choices."
            )
