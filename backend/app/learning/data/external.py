...
class ExternalDataStore:
    ...
    def __init__(self, storage_dir: Optional[str] = None):
        ...
        """
        self.storage_dir = storage_dir or str(Path(settings.LEARNING_DATA_DIR) / "external")
        ...
        logger.info("ExternalDataStore initialized", extra={"storage_dir": self.storage_dir})
    ...
    def save(self, data: Dict[str, Any]) -> str:
        ...
        """
        # Ajouter métadonnées
        meta_data = {"version": "4.1.0", "timestamp": datetime.utcnow().isoformat(), **data}
        ...
        logger.info("Meta data saved", extra={"timestamp": timestamp, "sources": list(data.get("sources", {}).keys())})
    ...
    def get_features_for_ml(self) -> pd.DataFrame:
        ...
        for prof_data in professions:
            if isinstance(prof_data, dict):
                ...
                features.append({"profession": prof_data, "popularity": 1.0, "win_rate": 0.5, "trending_up": True})
    ...
    def get_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        history_files = sorted(self.history_dir.glob("meta_*.json"), reverse=True)[:limit]
