...
class SkillAnalysisRequest(BaseModel):
    skill_id: int = Field(..., description="GW2 skill ID (numeric)")
...
@router.post(
    "/analyze/build-full",
    response_model=Dict[str, Any],
    summary="Analyze a build and optimize its equipment",
    description=(
        "Runs a full analysis of a build (specialization, traits, skills) combining "
        "LLM-based synergy analysis with a mathematical equipment optimization "
        "(stats presets, runes, sigils) via BuildEquipmentOptimizer and BuildAdvisorAgent."
    ),
)
async def analyze_build_full_endpoint(
    payload: BuildAnalysisRequest,
    current_user: User = Depends(get_current_active_user),
) -> Dict[str, Any]:
    logger.info(
        "AI full build analysis request",
        extra={
            "specialization_id": payload.specialization_id,
            "trait_ids": payload.trait_ids,
            "skill_ids": payload.skill_ids,
    ...
    try:
        ...
        logger.error(f"Full build analysis failed: {e}")
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"AI full build analysis failed: {e}",
        )
