...
class MetaAnalysisRequest(BaseModel):
    ...
    game_mode: str = Field(
        ...,
        description="Mode de jeu (zerg, raid_guild, roaming)"
    )
    profession: Optional[str] = Field(
        None,
        description="Profession à analyser (optionnel)"
    )
    include_api_data: bool = Field(
        False,
        description="Inclure les données de l'API GW2"
    )
...
class GW2DataImportRequest(BaseModel):
    ...
    data_types: list[str] = Field(
        default=["professions", "specializations", "traits"],
        description="Types de données à importer"
    )
    profession: Optional[str] = Field(
        None,
        description="Profession spécifique à importer (optionnel)"
    )
...
async def analyze_meta(request: MetaAnalysisRequest):
    ...
    """
    logger.info(
        f"Meta analysis requested for game_mode={request.game_mode}, "
        f"profession={request.profession}"
    )
    ...
    try:
        ...
        if not result.get("success"):
            raise HTTPException(
                status_code=500,
                detail=f"Meta analysis failed: {result.get('error', 'Unknown error')}"
            )
...
async def get_meta_snapshot(
    game_mode: str,
    profession: Optional[str] = Query(None, description="Profession à filtrer")
):
    """
    ...
    try:
        ...
        result = await agent.run({
            ...
            "time_range": 7  # Snapshot sur 7 jours
        })
        
        if not result.get("success"):
            raise HTTPException(
                status_code=500,
                detail="Failed to get meta snapshot"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get meta snapshot: {str(e)}"
        )
...
async def get_gw2_profession(profession_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch profession: {str(e)}"
        )
