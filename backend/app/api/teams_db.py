...
async def create_team(
    ...
    try:
        ...
        try:
            ...
            await collector.collect_team_creation(
                team_db.__dict__,
...
async def list_public_teams(
    ...
    try:
        service = TeamService(db)
        teams_db = await service.list_public_teams(
            skip=skip,
            limit=limit,
            game_mode=game_mode
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error listing public teams: {str(e)}"
        )
...
async def add_build_to_team(
    ...
    try:
        ...
        return {
            "message": "Build added to team successfully",
            "slot_id": slot.id,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error adding build to team: {str(e)}"
        )
...
async def remove_build_from_team(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error removing build from team: {str(e)}"
        )
...
async def get_team_count(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error counting teams: {str(e)}"
        )
