...
class TeamSlotDB(Base):
    ...
    team_composition_id: Mapped[str] = mapped_column(
        String, ForeignKey("team_compositions.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    # Relationships
    team_composition: Mapped["TeamCompositionDB"] = relationship("TeamCompositionDB", back_populates="team_slots")
...
class TeamCompositionCreate(TeamCompositionBase):
    ...
    build_ids: List[str] = Field(default_factory=list, description="List of build IDs to include in the team")
...
class TeamResponse(BaseModel):
    ...
    alternative_compositions: List[TeamComposition] = Field(
        default_factory=list, description="Alternative compositions"
    )
...
class TeamOptimizeRequest(BaseModel):
    ...
    game_mode: GameMode
    team_size: int = Field(ge=1, le=50, description="Team size")
    required_roles: Dict[str, int] = Field(default_factory=dict, description="Mapping of role names to count required")
    existing_builds: List[str] = Field(
        default_factory=list, description="List of build IDs to include in the optimization"
    )
    constraints: Optional[str] = Field(None, description="Additional constraints in natural language")
