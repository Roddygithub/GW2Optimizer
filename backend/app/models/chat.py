...
class BuildSuggestion(BaseModel):
    ...
    weapons: Optional[Dict[str, str]] = Field(
        None,
        description="Weapons used in the build"
    )
    ...
    player_count: Optional[int] = Field(
        None,
        description="Number of players using this build"
    )
    gw2skills_url: Optional[str] = Field(
        None,
        description="GW2Skills URL for the build (e.g., https://gw2skills.net/editor/abc123)"
    )
...
class ChatResponse(BaseModel):
    ...
    suggestions: List[str] = Field(
        default_factory=list,
        description="List of suggested follow-up questions or actions"
    )
    builds: List[BuildSuggestion] = Field(
        default_factory=list,
        description="List of build suggestions"
    )
    ...
    action_required: Optional[str] = Field(
        None,
        description="Action required from the user (e.g., 'parse_build', 'optimize_team')"
    )
