...
class TeamAnalysisWorkflow(BaseWorkflow):
    ...
    async def process_results(
        ...
        if optimization_result:
            ...
            final_result["comparison"] = self._compare_compositions(
                initial_analysis,
                optimization_result.get("after_analysis", {})
            )
            ...
            final_result["recommendations"] = self._generate_basic_recommendations(
                initial_analysis
            )
    ...
    def _compare_compositions(
        ...
        return {
            ...
            "boon_coverage_improvements": self._compare_boon_coverage(
                before.get("boon_coverage", {}),
                after.get("boon_coverage", {})
            )
        }
    
    def _compare_boon_coverage(
        self,
        before: Dict[str, str],
        after: Dict[str, str]
    ) -> List[str]:
    ...
    def _generate_basic_recommendations(
        ...
        poor_boons = [
            boon for boon, level in boon_coverage.items()
            if "Poor" in level or "Fair" in level
        ]
        
        if poor_boons:
            recommendations.append(
                f"⚠ Improve boon coverage for: {', '.join(poor_boons[:3])}"
            )
