...
class BuildOptimizationWorkflow(BaseWorkflow):
    ...
    def __init__(self):
        ...
        self.steps = [
            WorkflowStep(name="build_recommendation", agent_name="recommender", inputs={}),
            WorkflowStep(name="synergy_analysis", agent_name="synergy", inputs={}),
        ]
        ...
        if "team_composition" in inputs and inputs["team_composition"]:
            ...
            steps.append(
                WorkflowStep(
                    ...
                    agent_name="synergy",
                    inputs={"professions": team_comp, "game_mode": inputs["game_mode"]},
    ...
    async def process_results(self, step_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if synergy_analysis:
            ...
            final_result["synergy_recommendations"] = self._generate_synergy_recommendations(
                initial_build, synergy_analysis
            )

        # Comparaison des builds
        final_result["comparison"] = self._compare_builds(initial_build, build_variant)

        # Recommandation finale
        final_result["recommendation"] = self._generate_final_recommendation(
            initial_build, build_variant, synergy_analysis
        )
    ...
    def _generate_synergy_recommendations(self, build: Dict[str, Any], synergy_analysis: Dict[str, Any]) -> List[str]:
        """
        ...
        for weakness in weaknesses:
            ...
            if any(syn.lower() in weakness_lower for syn in build_synergies):
                recommendations.append(f"✓ This build helps address: {weakness}")
            else:
                recommendations.append(f"⚠ Consider adjusting for: {weakness}")
    ...
    def _compare_builds(self, build_a: Dict[str, Any], build_b: Dict[str, Any]) -> Dict[str, Any]:
        """
        Compare deux builds.
