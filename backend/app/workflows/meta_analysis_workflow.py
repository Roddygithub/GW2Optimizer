...
class MetaAnalysisWorkflow(BaseWorkflow):
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        """
        super().__init__(
            name="MetaAnalysisWorkflow",
            description="Workflow d'analyse complète du méta GW2",
            version="1.0.0"
        )
        ...
        self.workflow_steps = [
            {
                "name": "collect_game_data",
                "description": "Collecte des données de jeu via API GW2",
                "required": False
            },
            {
                "name": "analyze_meta",
                "description": "Analyse du méta actuel",
                "required": True
            },
            {
                "name": "detect_trends",
                "description": "Détection des tendances",
                "required": True
            },
            {
                "name": "generate_recommendations",
                "description": "Génération de recommandations",
                "required": True
            },
            {
                "name": "create_report",
                "description": "Création du rapport d'analyse",
                "required": True
            }
        ]
    ...
    async def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Starting Meta Analysis Workflow for game_mode={game_mode}, "
    ...
    def _create_executive_summary(
        ...
        avg_viability = (
            sum(viability_scores.values()) / len(viability_scores)
            if viability_scores else 0
        )
    ...
    def _assess_meta_stability(self, trends: List[Dict[str, Any]]) -> str:
        ...
        strong_trends = [
            t for t in trends 
            if t.get("change_percentage", 0) > 0.2
        ]
    ...
    def _extract_key_insights(
        ...
        if trends:
            ...
            insights.append(
                f"Tendance principale: {top_trend.get('description', 'N/A')}"
            )
        
        # Insight sur la viabilité
        if viability_scores:
            low_viability_count = sum(
                1 for score in viability_scores.values() if score < 0.5
            )
            if low_viability_count > 0:
                insights.append(
                    f"{low_viability_count} builds nécessitent une optimisation"
                )
        ...
        if high_priority:
            insights.append(
                f"{len(high_priority)} actions prioritaires recommandées"
            )
