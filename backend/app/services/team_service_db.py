...
class TeamService:
    ...
    async def create_team(
        ...
        try:
            ...
            for idx, build_id in enumerate(team_data.build_ids, start=1):
                slot = TeamSlotDB(
                    id=str(uuid4()),
                    team_composition_id=team_db.id,
                    build_id=build_id,
                    slot_number=idx,
                    priority=1
                )
            ...
            stmt = (
                ...
                .where(
                    and_(
                        ...
                        or_(
                            TeamCompositionDB.user_id == str(user.id),
                            TeamCompositionDB.is_public == True
                        )
                    )
                )
            )
    ...
    async def add_build_to_team(
        ...
        try:
            ...
            if slot_number is None:
                stmt = select(func.max(TeamSlotDB.slot_number)).where(
                    TeamSlotDB.team_composition_id == team_id
                )
                result = await self.db.execute(stmt)
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to add build to team: {str(e)}"
            )
    ...
    async def remove_build_from_team(
        ...
        try:
            ...
            # Get the slot
            stmt = select(TeamSlotDB).where(
                and_(
                    TeamSlotDB.id == slot_id,
                    TeamSlotDB.team_composition_id == team_id
                )
            )
            result = await self.db.execute(stmt)
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to remove build from team: {str(e)}"
            )
    ...
    async def count_user_teams(self, user: UserDB) -> int:
        ...
        """
        try:
            stmt = select(func.count(TeamCompositionDB.id)).where(
                TeamCompositionDB.user_id == str(user.id)
            )
            result = await self.db.execute(stmt)
