...
class TestChatServiceIntegration:
    ...
    async def test_process_message_success(self, chat_service):
        ...
        chat_service.ollama.chat.return_value = "This is a test response with some suggestions.\n- Suggestion 1\n- Suggestion 2"
        
        # Create a test request
        request = ChatRequest(
            message="What's the best build for WvW?",
    ...
    async def test_error_handling(self, chat_service):
        ...
        with pytest.raises(ValueError):
            request = ChatRequest(
                message="Test",
                conversation_history=[{"invalid": "data"}],
                version="1.0.0"
            )
            await chat_service.process_message(request)
