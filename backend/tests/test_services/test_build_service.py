...
class TestBuildService:
    ...
    async def test_get_build_by_owner(
        self, db_session: AsyncSession, test_user: UserDB, sample_build_data: dict
    ):
        ...
        assert retrieved_build.user_id == test_user.id
    ...
    async def test_list_user_builds(
        ...
        assert len(builds) == 2
        assert all(build.user_id == test_user.id for build in builds)
    ...
    async def test_list_user_builds_with_profession_filter(
        ...
        guardian_builds = await service.list_user_builds(
            test_user, profession=Profession.GUARDIAN
        )
    ...
    async def test_list_user_builds_with_game_mode_filter(
        ...
        zerg_builds = await service.list_user_builds(
            test_user, game_mode=GameMode.ZERG
        )
    ...
    async def test_count_user_builds(
        self, db_session: AsyncSession, test_user: UserDB, sample_build_data: dict
    ):
