...
class TestTeamComposition:
    ...
    def test_composition_to_dict(self):
        ...
        comp = TeamComposition(
            name="Test Comp", size=10, game_mode=GameMode.RAID, builds=[], strategy="Test", strengths=[], weaknesses=[]
        )
...
class TestGW2AICore:
    ...
    async def test_compose_team_with_custom_size(self, ai_core):
        ...
        comp = await ai_core.compose_team(game_mode=GameMode.RAID, team_size=15)
    ...
    async def test_fallback_composition(self, ai_core):
        ...
        comp = await ai_core._generate_fallback_composition(
            game_mode=GameMode.ZERG, team_size=50, preferences=None, request_id="test-123"
        )
    ...
    async def test_mistral_generation_success(self, ai_core):
        ...
        with patch.object(ai_core._client, "post", return_value=mock_response):
            comp = await ai_core.compose_team(game_mode=GameMode.RAID, team_size=10)
    ...
    async def test_mistral_generation_failure_with_fallback(self, ai_core):
        ...
        # Mock HTTP error
        with patch.object(ai_core._client, "post", side_effect=httpx.HTTPError("Connection failed")):
            comp = await ai_core.compose_team(game_mode=GameMode.RAID, team_size=10)
    ...
    async def test_mistral_generation_failure_without_fallback(self, ai_core):
        ...
        with patch.object(ai_core._client, "post", side_effect=httpx.HTTPError("Connection failed")):
            with pytest.raises(httpx.HTTPError):
                await ai_core.compose_team(game_mode=GameMode.RAID, team_size=10)
    ...
    def test_create_mistral_prompt(self, ai_core):
        ...
        prompt = ai_core._create_mistral_prompt(game_mode=GameMode.RAID, team_size=10, preferences={"focus": "boons"})
    ...
    async def test_preferences_passed_to_composition(self, ai_core):
        ...
        comp = await ai_core.compose_team(game_mode=GameMode.RAID, preferences=preferences)
    ...
    async def test_request_id_in_logs(self, ai_core):
        ...
        comp = await ai_core.compose_team(game_mode=GameMode.RAID, request_id="custom-request-123")
