...
async def test_get_build_public(db_session: AsyncSession, test_user: UserDB, build_data: BuildCreate):
    ...
    # Create another user
    other_user = UserDB(
        id="other-user-id", email="other@example.com", username="otheruser", hashed_password="hashed_password"
    )
...
async def test_update_build(db_session: AsyncSession, test_user: UserDB, build_data: BuildCreate):
    ...
    # Update build
    update_data = BuildUpdate(name="Updated Name", description="Updated description")
