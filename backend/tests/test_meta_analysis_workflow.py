...
class TestMetaAnalysisWorkflow:
    ...
    async def test_workflow_run_basic(self):
        ...
        result = await workflow.run({"game_mode": "zerg", "time_range": 30})
    ...
    async def test_workflow_with_profession(self):
        ...
        result = await workflow.run({"game_mode": "raid_guild", "profession": "Guardian", "time_range": 14})
    ...
    async def test_workflow_with_api_data(self, mock_get_profession):
        ...
        result = await workflow.run(
            {"game_mode": "zerg", "profession": "Guardian", "include_api_data": True, "time_range": 30}
        )
    ...
    async def test_workflow_executive_summary(self):
        ...
        result = await workflow.run({"game_mode": "roaming", "time_range": 30})
    ...
    async def test_workflow_with_builds(self):
        ...
        result = await workflow.run({"game_mode": "zerg", "current_builds": test_builds, "time_range": 30})
    ...
    async def test_workflow_invalid_game_mode(self):
        ...
        result = await workflow.run({"game_mode": "invalid_mode", "time_range": 30})
    ...
    async def test_workflow_missing_game_mode(self):
        ...
        result = await workflow.run({"time_range": 30})
    ...
    async def test_workflow_meta_stability_assessment(self):
        ...
        # Test shifting
        trends_shifting = [{"change_percentage": 0.25, "confidence": 0.8}]
    ...
    async def test_workflow_key_insights_extraction(self):
        ...
        trends = [{"description": "Augmentation des builds support", "confidence": 0.85}]
        ...
        viability_scores = {"build_1": 0.3, "build_2": 0.8, "build_3": 0.4}

        insights = workflow._extract_key_insights(trends, recommendations, viability_scores)
    ...
    async def test_workflow_game_data_summary(self):
        ...
        game_data = {
            ...
            "specializations": [{"id": 27, "name": "Dragonhunter"}, {"id": 62, "name": "Firebrand"}],
        }
    ...
    async def test_workflow_different_time_ranges(self):
        ...
        # Test avec 7 jours
        result_7d = await workflow.run({"game_mode": "zerg", "time_range": 7})
        assert result_7d["success"] is True
        assert result_7d["report"]["analysis_period"] == "7 days"

        # Test avec 90 jours
        result_90d = await workflow.run({"game_mode": "zerg", "time_range": 90})
    ...
    async def test_workflow_all_game_modes(self):
        ...
        for mode in game_modes:
            result = await workflow.run({"game_mode": mode, "time_range": 30})
