...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if self.is_production:
            security_headers["Strict-Transport-Security"] = (
                "max-age=31536000; includeSubDomains; preload"  # type: ignore
            )
    ...
    def _get_csp_header(self) -> str:
        ...
        csp_directives = [
            ...
            if self.is_production
            else "",  # Instructs browsers to upgrade HTTP requests to HTTPS.
        ]
...
def add_security_middleware(app, settings):
    ...
    is_testing = bool(getattr(settings, "TESTING", False)) or bool(
